...
static int
get_cursor_column(int const ifd, int const ofd)
{
    // Request cursor location and read response start (ESC [)
    char c = '\0';
    if (write_string(ofd, VTESC "6n", 5) || read_char(ifd, &c) || c != ESC ||
        read_char(ifd, &c) || c != '[') {
        return -1;
    }
    ...
    int cols = 0;
    while (c != 'R') {
        if (read_char(ifd, &c) || ((c != 'R') && (c < '0' || c > '9'))) {
            return -1;
        }
...
get_columns(ComlinState* const state)
{
    ...
    if (isatty(ofd) && (ioctl(ofd, TIOCGWINSZ, &ws) == -1 || ws.ws_col <= 0)) {
        ...
        if (!write_string(ofd, VTESC "999C", 6)) { // Go to the right margin
            int const cols = get_cursor_column(ifd, ofd); // Get the column
...
buf_append(StringBuf* const buf, char const* const s, size_t const len)
{
    ...
    if (!buf->data || needed_size > buf->size) {
        char* const new_data = (char*)realloc(buf->data, needed_size);
        if (!new_data) {
            return COMLIN_BAD_ALLOC;
        }
...
refresh_multi_line(ComlinState* const l, ComlinRefreshFlags const flags)
{
    ...
    if ((flags & REFRESH_CLEAN) && !(flags & REFRESH_WRITE)) {
        // Clean the top line
        snprintf(seq, 64, VTESC "0K");
        buf_append(&update, seq, strlen(seq));
    } else {
        // Write the prompt and the current buffer content
        buf_append(&update, l->prompt, l->plen);
...
comlin_edit_insert(ComlinState* const l, char const c)
{
    if (l->buf.length == l->pos) {
        ...
        ComlinStatus const st = buf_append(&l->buf, &c, 1U);
        if (st) {
            return st;
        }
        ...
        ComlinStatus const st = buf_append(&l->buf, " ", 1U);
        if (st) {
            return st;
        }
